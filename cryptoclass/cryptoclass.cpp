...
QString CryptoClass::getCHash(const QByteArray &data) const {
    ...
    char *s_bytes = NULL;
    char *cp = NULL;
    const char *passwd = data.constData();
    const char *magic = "6";
    const QByteArray bsalt = generateRndStr(16).toUtf8();
    const char *salt = bsalt.constData();
    ...
    if ((p_bytes = static_cast<char *>(OPENSSL_zalloc(passwd_len))) == NULL)
        goto err;
    ...
    if ((s_bytes = static_cast<char *>(OPENSSL_zalloc(salt_len))) == NULL)
        goto err;
